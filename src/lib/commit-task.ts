...
export interface WebhookCommitPayload {
  repository: string
  branch: string
  pusher: string
  projectId: string
...
export interface CreateTasksFromCommitsInput {
  projectId: string
  commits: string[]
  repository?: string
  branch?: string
  /** GitHub webhook pusher handle; resolved to a user via PUSHER_EMAIL_MAP. */
  pusher?: string
  /**
   * Explicit owner (assignee + creator). When provided it takes precedence over
   * `pusher` resolution — used for manual creation from the project UI, where
   * the logged-in user owns the task.
   */
  ownerId?: string | null
}

export type CreateTasksFromCommitsResult =
  | { ok: true; task: unknown }
  | { ok: false; status: number; error: string }

/**
 * Create a task (or, for multiple commits, one main task with the per-commit
 * tasks as todos) from GitHub commit URLs by analyzing each commit's diff with
 * the model. This is the single source of truth shared by the external
 * `/api/webhook-commit` endpoint and the in-app "create task from commit" flow,
 * so a manually pasted commit URL produces exactly the same result as a webhook.
 */
export async function createTasksFromCommits(
  input: CreateTasksFromCommitsInput
): Promise<CreateTasksFromCommitsResult> {
  const { projectId, commits } = input
  ...
  if (!Array.isArray(commits) || commits.length === 0) {
    return { ok: false, status: 400, error: "commits array is required" }
  }
  ...
  const payload: WebhookCommitPayload = {
  ...
  if (commitTasks.length === 0) {
    return { ok: false, status: 502, error: "Failed to generate task from commits" }
  }
  ...
  // Resolve the owning user: an explicit ownerId (manual creation) wins;
  // otherwise fall back to mapping the webhook pusher to a registered user.
  let ownerId: string | null = input.ownerId ?? null
  if (!ownerId && payload.pusher) {
  ...
  if (!main) {
    return { ok: false, status: 502, error: "Failed to synthesize main task" }
  }
